...
export default function CreateProject({ token, onSuccess, onCancel }) {
    ...
    try {
      const res = await fetch(`${API}/projects`, {
        ...
        body: JSON.stringify({
          ...
          stage: STAGE_MAP[form.stage] || 'Ideation',
          supportRequired: form.supportNeeded.join(', '),
          technologies: form.techStack.split(',').map(t => t.trim()).filter(Boolean),
          isPublic: true
        })
      });
